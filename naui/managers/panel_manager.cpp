...
std::vector<NauiPanel>& panels() {
    static std::vector<NauiPanel> instance; // constructed on first use
    return instance;
}
...
{
    NauiPanel panel{ title, nullptr, false, panel_flags, ImGuiWindowFlags_None, ImGuiDockNodeFlags_None, render_callback, init_callback };
...
void naui_panel_init_all()
{
    ...
    // Keep panels sorted by title for naui_panel_get (binary search)
    std::sort(panels().begin(), panels().end(), [](const NauiPanel& a, const NauiPanel& b) 
	{ 
...
{
    std::vector<NauiPanel*> popup_panels;
    std::vector<NauiPanel*> modal_panels;

    for (NauiPanel& panel : panels())
    {
        if (!panel.is_visible && !(panel.panel_flags & NauiPanelFlags_Popup) && !(panel.panel_flags & NauiPanelFlags_Modal))
            continue;

        if (panel.panel_flags & NauiPanelFlags_MenuBar)
        {
        ...
        else if (panel.panel_flags & NauiPanelFlags_Modal)
        {
            modal_panels.push_back(&panel);
        }
        else if (panel.panel_flags & NauiPanelFlags_Popup)
        {
    ...
    for (NauiPanel* panel : modal_panels)
    {
        if (panel->is_visible)
        {
    ...
    for (NauiPanel* panel : popup_panels)
    {
        if (panel->is_visible)
    ...
    ImGui::Begin("##PopNauiost", nullptr, host_flags);
...
void naui_panel_show(const char* title, bool visible)
{
    NauiPanel* panel = naui_panel_get(title);
...
{
    auto& vec = panels();
    auto it = std::lower_bound(vec.begin(), vec.end(), title, [](const NauiPanel& a, const char* key)
    {
